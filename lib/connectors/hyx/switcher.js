...
var Switcher = function(server, opts){
	...
	if(opts.ssl){
		...
		server.on('connection', newSocket.bind(self, opts.hostFilter));
	}
...
var newSocket = function(hostFilter, socket){
	var self = this;
	if(self.state !== ST_STARTED) return;
	if(!hostFilter(socket.remoteAddress)) return socket.destroy();
