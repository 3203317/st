...
proto.before = function(msg, session, next){
	...
	if(count > self.maxSize){
		console.info('[INFO ] [%s] timeout filter is out of range, current size is %s, max size is %s.', utils.format(), count, self.maxSize);
		next();
		return;
	}
	var curId = utils.genObjectId();
	self.timeouts[curId] = setTimeout(function(){
		console.warn('[WARN ] [%s] request %j timeout.', utils.format(), msg.__route__);
	}, self.timeout);
