...
var hostFilter = function(socket, cb){
	...
	if(!!blacklistFilter && ('function' === typeof blacklistFilter)){
		return blacklistFilter(ip, function (err, result){
			if(err){
...
var securityCheck = function(socket, cb){
	...
	if(!(connCount < connection.maxConnections)){
		console.warn('[%s] server %j has reached the max connections is %j'.yellow, utils.format(), curServer.getServerId(), connection.maxConnections);
		return cb.call(self, socket, false);
	} // END
	cb(self, socket, true);
};